...
export function SystemHeader({ project }: { project: Project }) {
  return <header className="dossier-header">
    ...
    {project.dossier.links?.length ? <div className="dossier-links">{project.dossier.links.map((link) => <a key={link.label} href={link.href} target="_blank" rel="noreferrer">{link.label} <span aria-hidden="true">↗</span></a>)}</div> : null}
